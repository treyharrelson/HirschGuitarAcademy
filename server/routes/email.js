...
async function sendValidationEmail(client, user) {
	...
	const { data, error } = await client.emails.send({
		...
		from: 'Hirsch Guitar Academy <onboarding@resend.dev>',
		to: [user.email],
		...
		html: `
			...
			<head>
				<style>
					...
					.header { background: linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%); padding: 40px; border-radius: 12px 12px 0 0; text-align: center; }
					.content { background: #ffffff; padding: 40px; border: 1px solid #e5e7eb; border-radius: 0 0 12px 12px; }
					.button { display: inline-block; background: #3b82f6; color: #ffffff !important; text-decoration: none; padding: 14px 28px; border-radius: 8px; font-weight: 600; margin: 24px 0; box-shadow: 0 4px 60px -1px rgba(59, 130, 246, 0.4); }
					.footer { text-align: center; margin-top: 24px; font-size: 14px; color: #6b7280; }
					h1 { color: #ffffff; margin: 0; font-size: 28px; letter-spacing: -0.025em; }
					p { line-height: 1.6; }
				</style>
...
module.exports = { resend, sendValidationEmail, sendConfirmedEmail };